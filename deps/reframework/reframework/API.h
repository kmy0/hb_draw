...
typedef bool (*REFOnMessageCb)(void *, unsigned int, unsigned long long,
                               long long);
...
typedef bool (*REFOnPreApplicationEntryFn)(const char *,
                                           REFOnPreApplicationEntryCb);
typedef bool (*REFOnPostApplicationEntryFn)(const char *,
                                            REFOnPostApplicationEntryCb);
...
DECLARE_REFRAMEWORK_HANDLE(
    REFrameworkReflectionPropertyHandle); /* NOT a TDB property */
DECLARE_REFRAMEWORK_HANDLE(
    REFrameworkReflectionMethodHandle); /* NOT a TDB method */
...
typedef void *(*REFrameworkReflectionPropertyMethod)(
    REFrameworkReflectionPropertyHandle prop,
    REFrameworkManagedObjectHandle thisptr, void *out_data);
...
typedef struct {
    ...
    REFrameworkResult (*get_full_name)(REFrameworkTypeDefinitionHandle,
    ...
    bool (*is_derived_from)(REFrameworkTypeDefinitionHandle,
                            REFrameworkTypeDefinitionHandle);
    bool (*is_derived_from_by_name)(REFrameworkTypeDefinitionHandle,
                                    const char *);
    ...
    /* so you can cache them on your end e.g. with a static var to get a minor
     * speed increase. */
    REFrameworkMethodHandle (*find_method)(REFrameworkTypeDefinitionHandle,
                                           const char *);
    REFrameworkFieldHandle (*find_field)(REFrameworkTypeDefinitionHandle,
                                         const char *);
    REFrameworkPropertyHandle (*find_property)(
        REFrameworkTypeDefinitionHandle,
        const char *); /* not implemented yet. */

    /* out_size is the full size, in bytes of the out buffer */
    /* out_len is how many elements were written to the out buffer, not the size
     * of the written data */
    REFrameworkResult (*get_methods)(REFrameworkTypeDefinitionHandle,
                                     REFrameworkMethodHandle *out,
                                     unsigned int out_size,
                                     unsigned int *out_count);
    REFrameworkResult (*get_fields)(REFrameworkTypeDefinitionHandle,
                                    REFrameworkFieldHandle *out,
                                    unsigned int out_size,
                                    unsigned int *out_count);
    ...
    REFrameworkManagedObjectHandle (*create_instance)(
        REFrameworkTypeDefinitionHandle, unsigned int flags);

    REFrameworkTypeDefinitionHandle (*get_parent_type)(
        REFrameworkTypeDefinitionHandle);
    REFrameworkTypeDefinitionHandle (*get_declaring_type)(
        REFrameworkTypeDefinitionHandle);
    REFrameworkTypeDefinitionHandle (*get_underlying_type)(
        REFrameworkTypeDefinitionHandle);

    REFrameworkTypeInfoHandle (*get_type_info)(REFrameworkTypeDefinitionHandle);
    REFrameworkManagedObjectHandle (*get_runtime_type)(
        REFrameworkTypeDefinitionHandle);
...
typedef struct {
    ...
    REFrameworkResult (*invoke)(REFrameworkMethodHandle, void *thisptr,
                                void **in_args, unsigned int in_args_size,
    ...
    /* out_size is the full size, in bytes of the out buffer */
    /* out_count is how many elements were written to the out buffer, not the
     * size of the written data */
    REFrameworkResult (*get_params)(REFrameworkMethodHandle,
                                    REFrameworkMethodParameter *out,
                                    unsigned int out_size,
                                    unsigned int *out_len);
...
typedef struct {
    ...
    void *(*get_data_raw)(REFrameworkFieldHandle, void *obj,
                          bool is_value_type);
...
typedef struct {
    ...
    /* so you can cache them on your end e.g. with a static var to get a minor
     * speed increase. */
    REFrameworkTypeDefinitionHandle (*get_type)(REFrameworkTDBHandle,
                                                unsigned int index);
    REFrameworkTypeDefinitionHandle (*find_type)(REFrameworkTDBHandle,
                                                 const char *name);
    REFrameworkTypeDefinitionHandle (*find_type_by_fqn)(REFrameworkTDBHandle,
                                                        unsigned int fqn);
    REFrameworkMethodHandle (*get_method)(REFrameworkTDBHandle,
                                          unsigned int index);
    REFrameworkMethodHandle (*find_method)(REFrameworkTDBHandle,
                                           const char *type_name,
                                           const char *name);
    REFrameworkFieldHandle (*get_field)(REFrameworkTDBHandle,
                                        unsigned int index);
    REFrameworkFieldHandle (*find_field)(REFrameworkTDBHandle,
                                         const char *type_name,
                                         const char *name);
    REFrameworkPropertyHandle (*get_property)(REFrameworkTDBHandle,
                                              unsigned int index);

    REFrameworkModuleHandle (*get_module)(REFrameworkTDBHandle,
                                          unsigned int index);
...
typedef struct {
    ...
    REFrameworkTypeDefinitionHandle (*get_type_definition)(
    ...
    REFrameworkTypeInfoHandle (*get_type_info)(
        REFrameworkManagedObjectHandle); /* NOT a type definition */
    void *(*get_reflection_properties)(REFrameworkManagedObjectHandle);
    REFrameworkReflectionPropertyHandle (*get_reflection_property_descriptor)(
        REFrameworkManagedObjectHandle, const char *name);
    REFrameworkReflectionMethodHandle (*get_reflection_method_descriptor)(
        REFrameworkManagedObjectHandle, const char *name);
...
typedef struct {
    ...
    const char *name; /* t is not guaranteed to be non-null so we pass the name
                         along too */
...
typedef struct {
    ...
    REFrameworkResourceHandle (*create_resource)(
        REFrameworkResourceManagerHandle, const char *type_name,
        const char *name);
    REFrameworkManagedObjectHandle (*create_userdata)(
        REFrameworkResourceManagerHandle, const char *type_name,
        const char *name);
...
typedef struct {
    ...
    REFrameworkManagedObjectHandle (*create_holder)(REFrameworkResourceHandle,
                                                    const char *type_name);
} REFrameworkResource;
...
typedef struct {
    ...
    REFrameworkReflectionPropertyHandle (*get_reflection_property_descriptor)(
        REFrameworkTypeInfoHandle, const char *name);
    REFrameworkReflectionMethodHandle (*get_reflection_method_descriptor)(
        REFrameworkTypeInfoHandle, const char *name);
...
typedef struct {
    ...
    void (*cleanup_after_exception)(REFrameworkVMContextHandle,
...
/* NOT a TDB property */
typedef struct {
    REFrameworkReflectionPropertyMethod (*get_getter)(
...
typedef int (*REFPreHookFn)(int argc, void **argv,
                            REFrameworkTypeDefinitionHandle *arg_tys,
                            unsigned long long ret_addr);
typedef void (*REFPostHookFn)(void **ret_val,
                              REFrameworkTypeDefinitionHandle ret_ty,
                              unsigned long long ret_addr);
...
typedef struct {
    ...
    REFrameworkManagedObjectHandle (*typeof_)(
        const char *type_name); /* System.Type */
    REFrameworkManagedObjectHandle (*get_managed_singleton)(
        const char *type_name);
    ...
    /* out_count is how many elements were written to the out buffer, not the
     * size of the written data */
    REFrameworkResult (*get_managed_singletons)(
        REFrameworkManagedSingleton *out, unsigned int out_size,
        unsigned int *out_count);
    REFrameworkResult (*get_native_singletons)(REFrameworkNativeSingleton *out,
                                               unsigned int out_size,
                                               unsigned int *out_count);

    REFrameworkManagedObjectHandle (*create_managed_string)(const wchar_t *str);
    REFrameworkManagedObjectHandle (*create_managed_string_normal)(
        const char *str);

    unsigned int (*add_hook)(REFrameworkMethodHandle, REFPreHookFn,
                             REFPostHookFn, bool ignore_jmp);
    void (*remove_hook)(REFrameworkMethodHandle, unsigned int);

    void *(*allocate)(unsigned long long size);
    void (*deallocate)(void *);

    REFrameworkManagedObjectHandle (*create_managed_array)(
        REFrameworkTypeDefinitionHandle, unsigned int size);
...
typedef struct {
    ...
    const REFrameworkReflectionProperty
        *reflection_property; /* NOT a TDB property */
